...
\subsection{2.役割:現在の状態がゴール状態であるかを判定するMazeProblem での実装：} 
\begin{lstlisting}
	public boolean isGoal() {
		return "E".equals(this.current);
	}
...
\subsection{3.役割： 現在の状態から実行可能なすべてのアクション（移動先）を返すMazeProblem での実装：}
\begin{lstlisting}
public List<Action> actions() {
    return this.map.getOrDefault(current, Collections.emptyList()).stream()
            .map(p -> (Action) new MazeAction(p))
...
\subsection{4.役割： 指定されたアクションを実行した後の新しい状態（後続状態）を返すMazeProblem での実装：}
\begin{lstlisting}
public World successor(Action action) {
    var a = (MazeAction) action;
    return new MazeWorld(a.next);
...
\begin{figure}[!hbt]
  \centering
  \includegraphics[bb=0 0 300 300,width=0.2\linewidth]{3-2result.png}
  \caption{実行結果}
  \label{fig:result}
\end{figure}
\vspace{1em}
